...
public class AndroidNativeFunctions : MonoBehaviour {
	...
	public static void ShowAlert(string message,string title,string positiveButton,string negativeButton,string neutralButton,UnityAction<DialogInterface> action,UnityAction<DialogInterface> negativeAction){
		if(Application.platform != RuntimePlatform.Android)
			return;
		currentActivity.Call("runOnUiThread", new AndroidJavaRunnable(() => {
			...
			if(!string.IsNullOrEmpty(negativeButton)){
				builder.Call<AndroidJavaObject>("setNegativeButton",negativeButton,new ShowAlertListener(negativeAction));
			}
